...
void *FSSkel::Construct(const v8::FunctionCallbackInfo<Value>& info)
{
	...
	if (info.Length() > 0) {
		if (!info[0].IsEmpty() && info[0]->IsInt32()) {
			obj->x = info[0]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
		}
...
JS_SKEL_GET_PROPERTY_IMPL(GetPropertyY)
{
	...
	info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), y.c_str()).ToLocalChecked());
}
...
JS_SKEL_GET_PROPERTY_IMPL(GetPropertyZ)
{
	...
	info.GetReturnValue().Set(String::NewFromUtf8Literal(info.GetIsolate(), "z"));
}
...
JS_SKEL_FUNCTION_IMPL(MyFunction2)
{
	...
	info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Error in MyFunction2()"));
}
