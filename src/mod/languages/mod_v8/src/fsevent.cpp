		...
		if ((obj = new FSEvent(info))) {
			...
			if (etype == SWITCH_EVENT_CUSTOM) {
				...
				if (switch_event_create_subclass(&event, etype, subclass_name.c_str()) != SWITCH_STATUS_SUCCESS) {
					info.GetIsolate()->ThrowException(String::NewFromUtf8(info.GetIsolate(), "Failed to create sub class").ToLocalChecked());
					delete obj;
					return NULL;
				}
			} else {
				if (switch_event_create(&event, etype) != SWITCH_STATUS_SUCCESS) {
					info.GetIsolate()->ThrowException(String::NewFromUtf8(info.GetIsolate(), "Failed to create event").ToLocalChecked());
					delete obj;
					return NULL;
				}
			}
			...
			if (event && !info[2].IsEmpty() && info[2]->BooleanValue(info.GetIsolate())) {
...
JS_EVENT_FUNCTION_IMPL(AddHeader)
{
	...
	if (info.Length() > 1) {
		...
		if (!info[2].IsEmpty() && info[2]->BooleanValue(info.GetIsolate())) {
...
JS_EVENT_FUNCTION_IMPL(GetHeader)
{
	...
	if (info.Length() > 0) {
		...
		if (info.Length() > 1 && !info[1].IsEmpty()) {
			idx = info[1]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
...
JS_EVENT_FUNCTION_IMPL(GetBody)
{
	...
	if ((body = switch_event_get_body(_event))) {
		info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), body).ToLocalChecked());
	} else {
...
JS_EVENT_FUNCTION_IMPL(GetType)
{
	...
	info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), js_safe_str(event_name)).ToLocalChecked());
}
...
JS_EVENT_FUNCTION_IMPL(Serialize)
{
	...
	if (isxml) {
		...
		if ((xml = switch_event_xmlize(_event, SWITCH_VA_NONE))) {
			...
			info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), js_safe_str(xmlstr)).ToLocalChecked());
			switch_xml_free(xml);
		...
		if (switch_event_serialize_json(_event, &buf) == SWITCH_STATUS_SUCCESS) {
			info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), js_safe_str(buf)).ToLocalChecked());
