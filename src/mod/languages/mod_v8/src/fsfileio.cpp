...
void *FSFileIO::Construct(const v8::FunctionCallbackInfo<Value>& info)
{
	...
	info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Invalid Args"));
	return NULL;
}
...
JS_FILEIO_FUNCTION_IMPL(Read)
{
	...
	if (info.Length() > 0) {
		bytes = info[0]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
	}
...
JS_FILEIO_FUNCTION_IMPL(GetData)
{
	...
	if (!_buflen || !_buf) {
		...
		info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), _buf).ToLocalChecked());
	}
}
...
JS_FILEIO_GET_PROPERTY_IMPL(GetProperty)
{
	...
	if (!strcmp(prop, "path")) {
		if (_path) {
			info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), _path).ToLocalChecked());
		} else {
			info.GetReturnValue().Set(false);
		}
