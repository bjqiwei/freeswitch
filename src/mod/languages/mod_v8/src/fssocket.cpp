...
JS_SOCKET_FUNCTION_IMPL(Connect)
{
	if (info.Length() == 2) {
		...
		port = info[1]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
...
{
	if (!this->_socket) {
		info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Socket is not active"));
		return;
	}
...
JS_SOCKET_FUNCTION_IMPL(ReadBytes)
{
	...
	if (info.Length() == 1) {
		...
		bytes_to_read = info[0]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
		...
		if (ret != SWITCH_STATUS_SUCCESS) {
			...
			info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), js_safe_str(this->_read_buffer)).ToLocalChecked());
			return;
		}
	}
...
JS_SOCKET_GET_PROPERTY_IMPL(GetProperty)
{
	...
	if (!strcmp(js_safe_str(*str), "address")) {
		...
		if (sa && switch_get_addr(tmp, sizeof(tmp), sa) && !zstr(tmp)) {
			info.GetReturnValue().Set(String::NewFromUtf8Literal(info.GetIsolate(), tmp));
		} else {
			info.GetReturnValue().Set(String::NewFromUtf8Literal(info.GetIsolate(), "unknown"));
		}
...
JS_SOCKET_SET_PROPERTY_IMPL(SetPropertyTimeOut)
{
	...
	switch_socket_timeout_set(this->_socket, value->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked());
}
