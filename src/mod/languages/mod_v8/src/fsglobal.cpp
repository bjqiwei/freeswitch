		...
		while (line) {
			...
			if ((val = strchr(line, '='))) {
				*val = '\0';
				...
				// Add data to hash
				args->Set(config_data->isolate->GetCurrentContext(), String::NewFromUtf8(config_data->isolate, line).ToLocalChecked(), String::NewFromUtf8(config_data->isolate, js_safe_str(val)).ToLocalChecked());
			}
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(FetchURLHash)
{
	...
	if (info.Length() > 0) {
		String::Utf8Value url(info.GetIsolate(), info[0]);
		...
		if (info.Length() > 1 && info[1]->IsString()) {
			...
			Local<Value> obj = info.GetIsolate()->GetCurrentContext()->Global()->Get(info.GetIsolate()->GetCurrentContext(), str).ToLocalChecked();
			...
			if (!obj.IsEmpty() && obj->IsArray()) {
				...
				info.GetIsolate()->GetCurrentContext()->Global()->Set(info.GetIsolate()->GetCurrentContext(), str, arguments);
				config_data.hashObject.Reset(info.GetIsolate(), arguments);
			} else {
				/* The var exists, but is wrong type - exit with error */
				info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Second argument is the name of an existing var of the wrong type"));
			...
			/* The var exists, but is wrong type - exit with error */
			info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Second argument is of the wrong type"));
		...
		if (!curl_handle) {
			info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Failed to initiate curl easy session."));
		...
		/* Return the hash */
		info.GetReturnValue().Set(config_data.hashObject.Get(info.GetIsolate()));
	} else {
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(FetchURL)
{
	...
	if (info.Length() >= 1) {
		...
		url = js_safe_str(*str);
		if (info.Length() > 1) {
			buffer_size = info[1]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
		}
		...
		if (config_data.buffer == NULL) {
			info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Failed to allocate data buffer."));
			switch_curl_easy_cleanup(curl_handle);
		...
		if (code == CURLE_OK) {
			config_data.buffer[config_data.bufferDataLength] = 0;
			info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), js_safe_str(config_data.buffer)).ToLocalChecked());
		} else {
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(GlobalSet)
{
	...
	info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "var name not supplied!"));
}
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(Include)
{
	...
	if (info.Length() < 1) {
		...
		string scriptPath = "";
		Handle<Value> val = info.GetIsolate()->GetCurrentContext()->Global()->Get(info.GetIsolate()->GetCurrentContext(), String::NewFromUtf8Literal(info.GetIsolate(), "scriptPath")).ToLocalChecked();
			...
			if (script_name && JSMain::FileExists(script_name)) {
				...
				if (js_file.length() > 0) {
					...
					if (script.IsEmpty()) {
						...
						info.GetReturnValue().Set(script.ToLocalChecked()->Run(info.GetIsolate()->GetCurrentContext()).ToLocalChecked());
					}
#else
					Handle<String> source = String::NewFromUtf8Literal(info.GetIsolate(), js_file.c_str());
		...
		info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Failed to include javascript file"));
	}
}
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(Sleep)
{
	...
	if (info.Length() > 0) {
		msec = info[0]->Int32Value(info.GetIsolate()->GetCurrentContext()).ToChecked();
	}

	if (msec) {
		switch_yield(msec * 1000);
	} else {
		info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "No time specified"));
	}
}
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(ApiExecute)
{
	...
	if (info.Length() > 0) {
		...
		info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), switch_str_nil((char *) stream.data)).ToLocalChecked());
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(Email)
{
	...
	if (info.Length() > 3) {
		String::Utf8Value str(info.GetIsolate(), info[3]);
		body = js_safe_str(*str);
	}

	if (info.Length() > 4) {
		String::Utf8Value str(info.GetIsolate(), info[4]);
		file = js_safe_str(*str);
	}

	if (info.Length() > 5) {
		String::Utf8Value str(info.GetIsolate(), info[5]);
		convert_cmd = js_safe_str(*str);
	}

	if (info.Length() > 6) {
		String::Utf8Value str(info.GetIsolate(), info[6]);
	...
	if (info.Length() > 1) {
		if (info[0]->IsObject()) {
			...
			if (!(jss_a = JSBase::GetInstance<FSSession>(obj_a))) {
				info.GetIsolate()->ThrowException(String::NewFromUtf8Literal(info.GetIsolate(), "Cannot find session A"));
				return;
			}
		}
		if (info[1]->IsObject()) {
...
JS_GLOBAL_FUNCTION_IMPL_STATIC(Id)
{
	...
	info.GetReturnValue().Set(String::NewFromUtf8(info.GetIsolate(), private_data->str_task_id.c_str()).ToLocalChecked());
}
