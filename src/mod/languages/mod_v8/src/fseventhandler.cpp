...
JS_EVENTHANDLER_FUNCTION_IMPL(SendEvent)
{
	if (info.Length() == 0) {
		...
		if (!info[0].IsEmpty() && info[0]->IsObject()) {
			FSEvent *evt = JSBase::GetInstance<FSEvent>(info[0]->ToObject(info.GetIsolate()->GetCurrentContext()).ToLocalChecked());
			...
			if (!evt || !(event = evt->GetEvent())) {
				...
				if (info.Length() > 1) {
					if (!info[1].IsEmpty() && info[1]->IsObject()) {
						...
						FSSession *sess = JSBase::GetInstance<FSSession>(info[1]->ToObject(info.GetIsolate()->GetCurrentContext()).ToLocalChecked());
...
JS_EVENTHANDLER_FUNCTION_IMPL(ExecuteApi)
{
	if (info.Length() > 0) {
		...
		info.GetReturnValue().Set(String::NewFromUtf8Literal(info.GetIsolate(), "-ERR"));
	}
}
...
JS_EVENTHANDLER_FUNCTION_IMPL(ExecuteBgApi)
{
	...
	info.GetReturnValue().Set(String::NewFromUtf8Literal(info.GetIsolate(), acs->uuid_str));
